...
public class ContreCadreTraverseCorniere extends ElementGenerique {
	...
	private final double margeEntreTraverseEtPremiereCote = conf.largeurContreCadreTraverseCorniere / 2; // <--- Empirique
	...
	private final double ordonneeBasTraverse = this.ordonneeHautTraverse + conf.hauteurContreCadreTraverseCorniere;

	private final double diametreTrous = conf.largeurContreCadreTraverseCorniere / 8; // INCORRECT
	private final String valeurDiametreTrous = "Ø9";

	private final double nbCotesAGauche = conf.nbPairesTrousIntermediairesHorizontaux * 2 + 5;
	private final double distanceEntreCentreTraverseEtExtremiteGaucheDessin = conf.demiLargeurGaucheContreCadreTraverseCorniere + this.margeEntreTraverseEtPremiereCote + (this.curUnderLineGap + this.taillePoliceCote + this.margeInterCote) * this.nbCotesAGauche;
	private final double distanceEntreCentreTraverseEtExtremiteDroiteDessin = conf.demiLargeurDroitContreCadreTraverseCorniere + this.margeEntreTraverseEtPremiereCote + this.curUnderLineGap + this.taillePoliceCote;
	...
	private void draw(MyCustomSvgEnhanced g){		
	    ...
	    g.drawLine(this.abscisseAxeTraverse - conf.demiLargeurGaucheContreCadreTraverseCorniere, this.ordonneeHautTraverse + conf.largeurContreCadreTraverseCorniere, this.abscisseAxeTraverse - conf.demiLargeurGaucheContreCadreTraverseCorniere, this.ordonneeBasTraverse - conf.largeurContreCadreTraverseCorniere);
	    g.drawLine(this.abscisseAxeTraverse + conf.demiLargeurDroitContreCadreTraverseCorniere, this.ordonneeHautTraverse, this.abscisseAxeTraverse + conf.demiLargeurDroitContreCadreTraverseCorniere, this.ordonneeBasTraverse);
	    g.drawLine(this.abscisseAxeTraverse - conf.demiLargeurGaucheContreCadreTraverseCorniere, this.ordonneeHautTraverse + conf.largeurContreCadreTraverseCorniere, this.abscisseAxeTraverse + conf.demiLargeurDroitContreCadreTraverseCorniere, this.ordonneeHautTraverse);
	    g.drawLine(this.abscisseAxeTraverse - conf.demiLargeurGaucheContreCadreTraverseCorniere, this.ordonneeBasTraverse - conf.largeurContreCadreTraverseCorniere, this.abscisseAxeTraverse + conf.demiLargeurDroitContreCadreTraverseCorniere, this.ordonneeBasTraverse);
		...
		Point p1_1 = new Point(this.abscisseAxeTraverse - conf.demiLargeurGaucheContreCadreTraverseCorniere, this.ordonneeHautTraverse);
		Point p1_2 = new Point(this.abscisseAxeTraverse + conf.demiLargeurDroitContreCadreTraverseCorniere, this.ordonneeHautTraverse);
		...
		g.drawDistanceCote(p2_1, p2_2, margeEntreTraverseEtPremiereCote, - conf.demiLargeurGaucheContreCadreTraverseCorniere / 2 - 10, ShiftMode.RIGHT);

		Point p3 = new Point(this.abscisseAxeTraverse, this.ordonneeBasTraverse);
		Point pTemp = new Point(p3);
		Point pTemp2 = new Point(pTemp);
		double curDistanceCotesLaterales = conf.demiLargeurGaucheContreCadreTraverseCorniere + this.margeEntreTraverseEtPremiereCote;
		int nbCotes = conf.nbPairesTrousIntermediairesHorizontaux + 5;
		for(int i = 1 ; i <= nbCotes ; i++){
			if(i == 1 || i == nbCotes)
				pTemp.move(0, - conf.ecartEntreExtremiteEtPremierTrouContreCadreTraverseCorniere);
			else if(i == 2 || i == nbCotes - 1){
				pTemp.move(0, - conf.ecartEntrePremierTrouEtDeuxiemeTrouTraverseCorniere);
				if(i == nbCotes - 1)
					g.drawDiameterCote(this.valeurDiametreTrous, pTemp, - Math.PI / 4, 40, ShiftMode.LEFT, 5);
			}
			else if(i == 3 || i == nbCotes - 2){
				pTemp.move(0, - conf.entreAxeLateralTraverseCorniere);
				if(i == 3)
					g.drawDistanceCote(pTemp, pTemp2, conf.demiLargeurDroitContreCadreTraverseCorniere + this.margeEntreTraverseEtPremiereCote);
			}
			else{
				pTemp.move(0, - conf.entreAxeCentralTraverseCorniere);
				if(i == 4)
					g.drawDistanceCote(pTemp, pTemp2, conf.demiLargeurDroitContreCadreTraverseCorniere + this.margeEntreTraverseEtPremiereCote);
			}
