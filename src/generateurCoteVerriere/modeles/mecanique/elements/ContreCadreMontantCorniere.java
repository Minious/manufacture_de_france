...
public class ContreCadreMontantCorniere extends ElementGenerique {
	...
	private final double margeEntreMontantEtPremiereCote = conf.largeurContreCadreMontantCorniere / 2; // <--- Empirique
	...
	private final double ordonneeBasMontant = this.ordonneeHautMontant + conf.hauteurContreCadreMontantCorniere;

	private final double diametreTrous = conf.largeurContreCadreMontantCorniere / 8; // INCORRECT
	...
	private final double distanceEntreCentreMontantEtExtremiteGaucheDessin = conf.demiLargeurGaucheContreCadreMontantCorniere + this.margeEntreMontantEtPremiereCote + (this.curUnderLineGap + this.taillePoliceCote + this.margeInterCote) * this.nbCotesAGauche;
	private final double distanceEntreCentreMontantEtExtremiteDroiteDessin = conf.demiLargeurDroitContreCadreMontantCorniere + this.margeEntreMontantEtPremiereCote + this.curUnderLineGap + this.taillePoliceCote;
	...
	public ContreCadreMontantCorniere(Conf conf){
		super(conf);
	}
	...
	private void draw(MyCustomSvgEnhanced g){		
	    ...
	    g.drawLine(this.abscisseAxeMontant - conf.demiLargeurGaucheContreCadreMontantCorniere, this.ordonneeHautMontant + conf.largeurContreCadreMontantCorniere, this.abscisseAxeMontant - conf.demiLargeurGaucheContreCadreMontantCorniere, this.ordonneeBasMontant - conf.largeurContreCadreMontantCorniere);
	    g.drawLine(this.abscisseAxeMontant + conf.demiLargeurDroitContreCadreMontantCorniere, this.ordonneeHautMontant, this.abscisseAxeMontant + conf.demiLargeurDroitContreCadreMontantCorniere, this.ordonneeBasMontant);
	    g.drawLine(this.abscisseAxeMontant - conf.demiLargeurGaucheContreCadreMontantCorniere, this.ordonneeHautMontant + conf.largeurContreCadreMontantCorniere, this.abscisseAxeMontant + conf.demiLargeurDroitContreCadreMontantCorniere, this.ordonneeHautMontant);
	    g.drawLine(this.abscisseAxeMontant - conf.demiLargeurGaucheContreCadreMontantCorniere, this.ordonneeBasMontant - conf.largeurContreCadreMontantCorniere, this.abscisseAxeMontant + conf.demiLargeurDroitContreCadreMontantCorniere, this.ordonneeBasMontant);
		...
		Point p1_1 = new Point(this.abscisseAxeMontant - conf.demiLargeurGaucheContreCadreMontantCorniere, this.ordonneeHautMontant);
		Point p1_2 = new Point(this.abscisseAxeMontant + conf.demiLargeurDroitContreCadreMontantCorniere, this.ordonneeHautMontant);
		...
		g.drawDistanceCote(p2_1, p2_2, margeEntreMontantEtPremiereCote, - conf.demiLargeurGaucheContreCadreMontantCorniere / 2 - 10, ShiftMode.RIGHT);
		
		// Cote entre les trous et l'extrémité de la Corniere + affichage des trous
		double curDistanceCotesLaterales = conf.demiLargeurGaucheContreCadreMontantCorniere + this.margeEntreMontantEtPremiereCote;
		
		Point p3_1 = new Point(this.abscisseAxeMontant, this.ordonneeBasMontant);
	    Point p3_2 = new Point(this.abscisseAxeMontant, this.ordonneeBasMontant - conf.ecartEntreExtremiteEtPremierTrouContreCadreMontantCorniere);
		...
		Point p4 = new Point(this.abscisseAxeMontant, this.ordonneeBasMontant - conf.ecartEntreExtremiteEtPremierTrouContreCadreMontantCorniere - conf.ecartEntrePremierTrouEtDeuxiemeTrouMontantCorniere);
		g.drawDistanceCote(p3_1, p4, curDistanceCotesLaterales, 0, ShiftMode.LEFT);
		curDistanceCotesLaterales = decalerCote(curDistanceCotesLaterales);
		g.drawCircle(p4, this.diametreTrous);
		
		Point pTemp = new Point(this.abscisseAxeMontant, this.ordonneeBasMontant - conf.ecartEntreExtremiteEtPremierTrouContreCadreMontantCorniere - conf.ecartEntrePremierTrouEtDeuxiemeTrouMontantCorniere - conf.entreAxeMontant);
		g.drawDistanceCote(pTemp, p4, conf.demiLargeurDroitContreCadreMontantCorniere + this.margeEntreMontantEtPremiereCote);
